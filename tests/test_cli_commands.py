...
class TestCommentsCheck:
    ...
    def test_annotations_diff(self, tmp_path: Path) -> None:
        ...
        # Source has label "init"; target has differently-named "boot"
        ...
        assert result.exit_code == 0, result.output
        # Both label names appear in the diff row.
        assert "init" in result.output
        assert "boot" in result.output
        assert "differs" in result.output

    def test_annotations_diff_no_graph(self, tmp_path: Path) -> None:
        runner = CliRunner()
        _init_project(tmp_path, runner, "demo", "demo")
        result = runner.invoke(
            main,
